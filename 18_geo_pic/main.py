...
def check_callback(callback):
    ...
    if key == "types":
        ...
        if value == "plus":
            user_dct[user_id]["spn"] = round(user_dct[user_id]["spn"] / 2, 4)
        elif value == "minus":
            user_dct[user_id]["spn"] = round(user_dct[user_id]["spn"] * 2, 4)
    elif key == "move":
        user_dct[user_id]["lon"] = str(round(float(move_dct[value][0]), 6))
        user_dct[user_id]["lat"] = str(round(float(move_dct[value][1]), 6))
...
def send_map_with_buttons(user_id, user_dct):
    ...
    types = user_dct[user_id].get("types", "map")
    spn = user_dct[user_id].get("spn", 0.01)
    lat = user_dct[user_id].get("lat", "31.105141")
    lon = user_dct[user_id].get("lon", "121.014223")
